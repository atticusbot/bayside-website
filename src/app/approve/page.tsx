...
function ApproveForm() {
  ...
  const wrap  = { minHeight: '100vh', background: '#0a1e1a', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '40px 20px' }
  const card  = { background: '#fff', maxWidth: '520px', width: '100%', padding: '48px 40px' }
  const tag   = { fontFamily: mono, fontSize: '11px', letterSpacing: '0.2em', color: '#3dffa0', background: '#0a1e1a', display: 'inline-block', padding: '4px 12px', marginBottom: '20px' }

  if (status === 'done') return (
    <div style={wrap}>
      <div style={{ ...card, textAlign: 'center' }}>
        <div style={tag}>[ CONFIRMED ]</div>
        <h1 style={{ fontFamily: mono, fontSize: '22px', color: '#0a1e1a', marginBottom: '16px', lineHeight: 1.3 }}>
        ...
        <p style={{ fontSize: '13px', color: '#6b8a82', marginTop: '24px' }}>
          Questions? Email <a href="mailto:tyler@baysideai.co" style={{ color: '#0a1e1a' }}>tyler@baysideai.co</a>
        </p>
      </div>
    </div>
  )
    ...
    <div style={wrap}>
      <div style={card}>
        <div style={{ marginBottom: '32px' }}>
          ...
          <h1 style={{ fontFamily: mono, fontSize: '22px', color: '#0a1e1a', lineHeight: 1.3, marginBottom: '12px' }}>
            {"Let's get started."}
          </h1>
          <p style={{ fontSize: '15px', color: '#3d5a54', lineHeight: 1.7 }}>
        ...
        <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
          ...
          <button type="submit" disabled={status === 'sending'}
            style={{ background: '#3dffa0', color: '#0a1e1a', fontFamily: mono, fontSize: '12px', fontWeight: 700, letterSpacing: '0.12em', textTransform: 'uppercase', padding: '16px 24px', border: 'none', cursor: 'pointer', opacity: status === 'sending' ? 0.6 : 1 }}>
